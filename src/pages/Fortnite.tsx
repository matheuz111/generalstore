...
const Fortnite = () => {
  ...
  useEffect(() => {
    const loadShop = async () => {
      try {
        const data = await fetchShopDual("ES");
        setSections(data);
      } catch (error) {
  ...
  if (loading) return <div className="text-white text-center py-20 font-bold">Cargando tienda en tiempo real...</div>;

  return (
    <CategoryShell
      title="Tienda de Fortnite"
      subtitle="Objetos disponibles hoy"
      banner="/images/fortnite.jpg"
    >
      <div className="space-y-12">
        {sections.map((section, idx) => (
          <div key={idx}>
            <h2 className="text-2xl font-black uppercase mb-6 text-blue-400" style={{ fontFamily: 'BurbankBig' }}>
              {section.titleEs}
            </h2>
            
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {section.items.map((item: any) => {
                // Calculamos los valores una sola vez para evitar errores
                ...
                return (
                  ...
                  >
                    ...
                    {/* PRECIO ALINEADO (Sustituye a los V-Bucks y al precio doble) */}
                    <div className="mt-auto flex items-center justify-between">
                      <p className="text-blue-300 font-black text-xl tracking-tight">
                        {priceFormatted}
                      </p>

                      {/* Icono pequeño de la rareza para mantener el estilo Fortnite */}
